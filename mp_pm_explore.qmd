...
q_flighthistory <- "SELECT *
                    FROM CT_FLIGHT_HISTORY WHERE FLIGHT_DATE BETWEEN '2024-08-01' AND '2024-08-31';"  # Define SQL query to select flight history between specified dates
...
clean_flighthistory <- view_flighthistory %>%  # Start processing the fetched flight history
  ...
  #filter(!duplicated(UNIQUE_ID)) %>%  # comment out
...
q_flightleg <- "SELECT *
                FROM CT_FLIGHT_LEG WHERE PAIRING_DATE BETWEEN '2024-07-25' AND '2024-08-31';"  # Define SQL query to select flight leg data for specified dates
...
flight_leg_join <-  view_flightleg %>%  # Start processing the fetched flight leg data
  mutate(updated_dt = paste(UPDATE_DATE, UPDATE_TIME, sep= " "),  # Create a new datetime column
         #DEADHEAD = if_else(is.na(DEADHEAD), "C", DEADHEAD), # Commented out: conditionally replace NA values in DEADHEAD
...
correct_fh <- clean_flighthistory %>%  # Start with cleaned flight history
  inner_join(flight_leg_join, by = join_by(FLIGHT_NO, DEPARTING_CITY, ARRIVAL_CITY, SCHED_DEPARTURE_DATE, SCHED_ARRIVAL_DATE),  # Perform an inner join with flight leg data based on specified columns
...
q_masterpairing <- "SELECT *
                    FROM CT_MASTER_PAIRING WHERE PAIRING_DATE BETWEEN '2024-07-25' AND '2024-08-31';"  # Define SQL query to select master pairing data
...
#   filter(PAIRING_STATUS == "A",  # Filter for active pairings
#          updated_dt == max(updated_dt)) %>%  # Keep only the latest update for each group
...
## Search for codes to remove false actives
```{r}

master_history_q <- "select CREW_INDICATOR, CREW_ID, BID_PERIOD, TRANSACTION_CODE, PAIRING_NO, PAIRING_DATE, TO_DATE, PAIRING_POSITION, PRIOR_TXN_CODE,
...
false_active_emp <- view_masterhistory %>% 
  ...
  filter(#!duplicated(temp_id),
         TRANSACTION_CODE %in% c("DRP", "FAR", "REM", "RFU"))
...
t <- anti_join(false_active_emp, final_pairing, by = c("PAIRING_DATE", "PAIRING_POSITION", "CREW_ID")) %>% 
  group_by(PAIRING_POSITION, CREW_ID, PAIRING_DATE) %>% 
  mutate(temp_id = cur_group_id()) %>% 
  filter(!duplicated(temp_id))
